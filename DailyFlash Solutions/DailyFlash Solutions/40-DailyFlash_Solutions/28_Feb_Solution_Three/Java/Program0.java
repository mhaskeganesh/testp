...
class Disarium {
	...
	for(int lc = 1; lc <= cnt; lc++){
		...
		sum = sum + Disarium.power(digits[lc],(cnt-lc+1));
	}

	//If-else to check for Dissarium Number
	if(num_input_1 == sum)
		System.out.printf("The Entered Number %d is Disarium Number\n",num_input_1);
	else
		System.out.printf("The Entered Number %d is not a Disarium Number\n",num_input_1);
